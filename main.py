...
class ComponentTimer:
    ...
    def print_summary(self):
        ...
        logger.info("=" * 50)
        total = sum(self.timings.values()) or 1
        for k, v in self.timings.items():
            logger.info(f"{k:28} | {v:7.2f}s | {v/total*100:5.1f}%")
        logger.info("=" * 50)
...
def build_vectorstore(documents: List[Document]) -> Weaviate:
    ...
    timer.start_timer("Vector Indexing & Storing (Weaviate)")
    client = weaviate.Client("http://localhost:8080")

    vectorstore = Weaviate.from_documents(chunks, embeddings, client=client)
    timer.end_timer("Vector Indexing & Storing (Weaviate)")
